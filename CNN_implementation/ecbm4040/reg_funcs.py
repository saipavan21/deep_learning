...
def dropout_forward(x, dropout_config, mode):
    ...
    out, cache = None, None
    dropout_x = np.random.choice([0,1], size=x.shape[0], p=[1-keep_prob, keep_prob])
    #drop_matrix = dropout_x
    #for i in range(x.shape[1]-1):
    #    drop_matrix = np.vstack((drop_matrix, dropout_x))
    drop_matrix = np.tile(dropout_x, (x.shape[1],1))
    drop_mask = drop_matrix.T
    if mode == "train":
        ###########################################
