    ...
    with tf.Session() as sess:
        ...
        for epc in range(epoch):
            ...
            for itr in range(iters):
                ...
                training_batch_x = X_train[itr * batch_size: (1 + itr) * batch_size]
                training_batch_y = y_train[itr * batch_size: (1 + itr) * batch_size]
                batch_generator = ImageGenerator(training_batch_x, training_batch_y)
                if epc % 5==0:
                  training_batch_x = training_batch_x
                elif epc % 5 ==3:
                  batch_generator.rotate(angle=-20)
                  #training_batch_x = batch_generator.x
                  training_batch_x =training_batch_x
                elif epc % 5 ==2:
                  batch_generator.flip(mode='hv')
                  training_batch_x = batch_generator.x
                elif epc % 5 ==1:
                  batch_generator.add_noise(portion=1, amplitude=1e-5)
                  training_batch_x = batch_generator.x
                else:
                  batch_generator.translate(6,-6)
                  training_batch_x = batch_generator.x
