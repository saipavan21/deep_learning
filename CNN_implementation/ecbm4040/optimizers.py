...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        ###################################################
        for k in grads:
            momentums[k] = (beta1 * momentums[k]) + ((1.0-beta1) * grads[k])
            velocitys[k] = (beta2 * velocitys[k]) + ((1.0-beta2) * grads[k]**2)
            momentum_k_hat = momentums[k]/(1.0-beta1**(t))
            velocity_k_hat = velocitys[k]/(1.0-beta2**(t))
            params[k] -= (learning_rate * momentum_k_hat)/(np.sqrt(velocity_k_hat) + eps)
